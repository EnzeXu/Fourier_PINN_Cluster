...
class Config:
    def __init__(self):
        self.model_name = "Toggle_Fourier_Lambda"
    ...
    def pend(self, y, t):
        k = self.params
        dydt = np.asarray([
            k.alpha_1 / (1 + y[1] ** k.beta) - y[0],
            k.alpha_2 / (1 + y[0] ** k.gamma) - y[1],
        ])
        return dydt
...
class FourierModel(nn.Module):
    ...
    def ode_gradient(self, x, y):
        ...
        f_U = - U_t + k.alpha_1 / (1 + V ** k.beta) - U
        f_V = - V_t + k.alpha_2 / (1 + U ** k.gamma) - V
    ...
    def write_finish_log(self):
        ...
        with open("saves/record.txt", "a") as f:
            f.write("{0}\t{1}\tseed={2}\t{3:.2f}min\titer={4}\tloss={5:.12f}\treal_loss_mse={6:.12f}\treal_loss_nmse={7:.12f}\tactivation={8}\tpenalty={9}\tstrategy={10}\tpinn={11}\n".format(
